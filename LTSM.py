...
class LSTMText(nn.Module):
    def __init__(self, input_dim, embedding_dim, hidden_dim, output_dim, pad_idx, dropout=0.5):
        ...
        # classifier now takes hidden_dim (not 2*hidden_dim)
        ...
        # ensure PAD row is exactly zero
        with torch.no_grad():
            self.embedding.weight[pad_idx].zero_()

    def forward(self, text, text_lengths):
        # text: [seq_len, batch]
        embedded = self.dropout(self.embedding(text))  # [seq_len, batch, emb]

        # pack by lengths so LSTM ignores padding
        packed = nn.utils.rnn.pack_padded_sequence(
            embedded, text_lengths.cpu(), enforce_sorted=False
        )
        _, (h_n, _) = self.lstm(packed)  # h_n: [1, batch, hidden_dim]

        x = h_n[-1]  # last layer’s hidden state, shape [batch, hidden_dim]

        x = self.dropout(self.relu(self.fc1(x)))
        x = self.dropout(self.relu(self.fc2(x)))
        x = self.dropout(self.relu(self.fc3(x)))
        logits = self.fc4(x)            # [batch, 1] if OUTPUT_DIM=1, else [batch, OUTPUT_DIM]
...
model = LSTMText(INPUT_DIM, EMBEDDING_DIM, HIDDEN_DIM, OUTPUT_DIM, PAD_IDX, dropout=0.5).to(device)
